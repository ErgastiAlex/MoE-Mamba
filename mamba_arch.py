...
class SS2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        # # The softmax is applied over the column because in each row there is a different token. A row only "sees" one token. A column "sees" each token
        # D = F.softmax(D, dim=1)
        # xs = einops.einsum(D, xs, "b k l2 l1, b k d l2 -> b k d l1")
        # # xs = D.T @ xs
        ...
        # C = einops.einsum(out_y, out_proj_w, "b d l1, b d l2 -> b l1 l2")
        # C = F.softmax(C, dim=-1)
